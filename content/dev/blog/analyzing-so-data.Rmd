...
[As we discussed previously](../devs-as-users/), we recently did an exercise of loading the Stack Overflow developer
survey](https://insights.stackoverflow.com/survey/2017) into Crunch so that our development team could experience the product as real users with real questions they wanted to answer. The first challenge that everyone encountered was "how do we get the data in and make it usable?"

Survey data is much more than a series of 1s and 2s in a CSV file. Metadata, including the wording of the questions asked and the response options given, is essential to being able to interpret the results of a survey, but it is lost or ignored in many file formats. In addition to that labeling metadata, surveys often involve data structures, such as multiple-response sets, which map intuitively to human concepts ("Which of the following do you agree with? Select all that apply.") but can be challenging to view and analyze properly in a flat, rectangular data export. Crunch was developed to make it
easier to work with these kinds of data structures, but going from a raw CSV to a well-labeled, cleaned dataset can take some work.

However, with some decent data wrangling skills, this process can be scripted efficiently. In this post, our R developers discuss how they used Crunch's R packages to harness the flexibility of Crunch's APIs along with the power of R to script repetitive cleaning tasks, reshape the data in ways that the web application doesn't let you do, and use statistical modeling to correct biases in the underlying data. We go from a raw CSV of values to a polished dataset that nontechnical users can explore and gain insights from on the web, while still maintaining the ability to do any statistical analysis we want in R.
...
To begin, let's load the [crunch](http://crunch.io/r/crunch/) package along with some others we'll use in the data processing.
...
Now, read in the CSVs from the survey and upload the data to the Crunch platform.
...
ds <- newDataset(stack_df, name = "StackOverflow Developer Survey 2017")
```

In the Crunch web app, we can take a look at some variable summaries to start to get a sense of the data structure.
...
Another common survey response data type can be found in Q350
