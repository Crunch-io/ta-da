...
Tidyeval has a great solution to this problem which is to bundle the expression and its environment in a single object called a "quosure". What this means it that as a developer you don't have to worry about matching expressions to environments and you can pass unevaluated expressions between functions with confidence. Because the expression and the environment are bundled together, when you end up evaluating it you won't be surprised by the result. 

I recently got a chance to work with tidy eval by developing autoplot methods for Crunch objects. One of these objects is called a `CrunchCube` which is an n-dimensional array of survey cross tabs. What I wanted to do was develop a plotting method which automatically adjusted to the dimensionality of the array to provide an appropriate visualization each type of cube. To provide a simple example of this kind of problem, let's create an `multiplot` method that plots things based on the grouping variables of a grouped_tbl. 
...
The idea here is to have a single function which will produce different plots based on the number of grouping variables. So the first step is to create a general plotting function which figures out which plotting method to use: 
...
What happens when you have more than three dimensions? Happily ggplot allows us to use tidyeval to dynamically add facets.
