...
u <- urls(datasets(yg))
length(u) ## 415
ids <- vapply(u, function (x) tail(unlist(strsplit(x, "/")), 1), character(1), USE.NAMES=FALSE)
logs <- paste0(ids, ".log")
...
sums <- lapply(since_october, summarize.elb)
sumdf <- do.call(rbind, lapply(sums, as.data.frame, stringsAsFactors=FALSE))
summary(sumdf)
head(sumdf)
